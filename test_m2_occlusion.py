"""
test_m2_occlusion.py
====================
Standalone smoke-test for M2 Occlusion metric.

Pretrained models: Mask2Former (COCO panoptic) → SegFormer → DeepLabV3 + skin

Usage
-----
python test_m2_occlusion.py --max_batches 2
python test_m2_occlusion.py --device cpu --max_batches 4
"""
...
def _probe_m2(device, batches):
    print(f"\n  Loading M2 Occlusion metric (device={device})...")
...
def main():
    p = argparse.ArgumentParser(description="Smoke-test M2 Occlusion metric")
    ...
    print("\n" + "=" * 90)
    print("  M2 Occlusion Metric — Standalone Smoke-Test")
    ...
    try:
        result, backend, dt = _probe_m2(args.device, batches)
        print(f"\n  {_g('LOADED')}  M2 Occlusion  (backend={backend}, {dt:.1f}s)")
        print("  Computed values:")
        for k in sorted(result):
            print(f"    {k:40s} = {_fv(result[k])}")
        status = 0
    except Exception as e:
        print(f"\n  {_r('NOT LOADED')}  M2 Occlusion")
