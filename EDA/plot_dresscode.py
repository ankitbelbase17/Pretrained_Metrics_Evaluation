...
def main() -> None:
    p = argparse.ArgumentParser(description="Plot DressCode EDA figures from cache")
    p.add_argument("--cache_root", type=str, default="cache_root")
    p.add_argument("--out_dir", type=str, default="assets")
    p.add_argument("--error_log", type=str, default="logs/plot_dresscode_errors.log")
    p.add_argument("--no_pairplot", action="store_true")
    args = p.parse_args()

    try:
        cache_root = Path(args.cache_root)
        cache_path = find_dataset_cache(cache_root, "dresscode")
        if cache_path is None:
            raise FileNotFoundError(f"Could not find DressCode cache under {cache_root}")

        all_data = {"DressCode": load_npz_dict(cache_path)}
        run_all_plots(all_data, out_root=args.out_dir, no_pairplot=args.no_pairplot)
    except Exception as e:
        log_plot_error("plot_dresscode", args.error_log, e)
