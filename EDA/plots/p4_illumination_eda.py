...
Illumination EDA — ECCV Publication Figures
  ...
  Figure 4B:  PCA of illumination maps → 2D scatter revealing lighting modes
              (Z_light — no variance barchart inset)
...
def _pca_2d(X_scaled: np.ndarray, n_components: int = 2) -> tuple:
...
def plot_luminance_spectrum(
    ...
    fig.suptitle("Illumination Complexity Analysis", fontsize=11, fontweight="bold", y=1.12)
...
def plot_illumination_pca(
    ...
    """
    ECCV-style PCA scatter of illumination maps (Z_light).
    Reveals distinct lighting modes across datasets.
    Variance barchart inset removed per user request — clean scatter only.
    """
    labels_all, mats = [], []
    ...
    ax.set_title("PCA of Illumination Maps ($Z_{light}$)", fontsize=10, fontweight="bold", pad=8)
