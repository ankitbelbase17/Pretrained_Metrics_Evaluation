"""
test_m4_illumination.py
=======================
Standalone smoke-test for M4 Illumination metric.

No pretrained model — uses signal processing (Lab luminance + Sobel gradients).

Usage
-----
python test_m4_illumination.py --max_batches 2
python test_m4_illumination.py --device cpu --max_batches 4
"""
...
def _probe_m4(device, batches):
    print(f"\n  Loading M4 Illumination metric...")
    from pretrained_metrics.metrics.m4_illumination import IlluminationMetrics
    obj = IlluminationMetrics()

    # Verify compute() is a method
    assert hasattr(obj, "compute"), "IlluminationMetrics.compute() not found!"
    assert callable(obj.compute), "IlluminationMetrics.compute is not callable!"
    print(f"  compute() method verified: OK")
    print(f"  Settings: blur_sigma={obj.blur_sigma}, use_mad={obj.use_mad}, normalize_grad={obj.normalize_grad}")
    ...
    result = obj.compute()
    dt = time.time() - t0
    return result, "signal_processing", dt

def main():
    p = argparse.ArgumentParser(description="Smoke-test M4 Illumination metric")
    ...
    print("\n" + "=" * 90)
    print("  M4 Illumination Metric — Standalone Smoke-Test")
    ...
    try:
        result, backend, dt = _probe_m4(args.device, batches)
        print(f"\n  {_g('LOADED')}  M4 Illumination  ({backend}, {dt:.1f}s)")
        print("  Computed values:")
        for k in sorted(result):
            print(f"    {k:40s} = {_fv(result[k])}")
        status = 0
    except Exception as e:
        print(f"\n  {_r('NOT LOADED')}  M4 Illumination")
