"""
test_m7_garment_texture.py
==========================
Standalone smoke-test for M7 Garment Texture Diversity metric.

Pretrained models: open_clip ViT-B/32 → ViT-B/16 (timm)

NOTE: M7 uses the CLOTH tensor, not the PERSON tensor.

Usage
-----
python test_m7_garment_texture.py --max_batches 2
python test_m7_garment_texture.py --device cpu --max_batches 4
"""
...
def _probe_m7(device, batches):
    print(f"\n  Loading M7 Garment Texture metric (device={device})...")
    ...
    embed_dim = getattr(obj._encoder, "embed_dim", "?")
    print(f"  Backend: {backend}  (embed_dim={embed_dim})")

    n_imgs = sum(b["cloth"].shape[0] for b in batches)
    print(f"  Running update on {n_imgs} CLOTH images...")
    t0 = time.time()
    for b in batches:
        obj.update(b["cloth"])  # M7 uses cloth, NOT person
    result = obj.compute()
    dt = time.time() - t0
    return result, backend, dt

def main():
    p = argparse.ArgumentParser(description="Smoke-test M7 Garment Texture metric")
    ...
    print("\n" + "=" * 90)
    print("  M7 Garment Texture Metric — Standalone Smoke-Test")
    ...
    try:
        result, backend, dt = _probe_m7(args.device, batches)
        print(f"\n  {_g('LOADED')}  M7 Garment Texture  (backend={backend}, {dt:.1f}s)")
        print("  Computed values:")
        for k in sorted(result):
            print(f"    {k:40s} = {_fv(result[k])}")
        status = 0
    except Exception as e:
        print(f"\n  {_r('NOT LOADED')}  M7 Garment Texture")
