"""
test_m3_background.py
=====================
Standalone smoke-test for M3 Background Complexity metric.

Pretrained models: DeepLabV3 (person segmentation) + DETR (object detection)

Usage
-----
python test_m3_background.py --max_batches 2
python test_m3_background.py --device cpu --max_batches 4
"""
...
def _probe_m3(device, batches):
    print(f"\n  Loading M3 Background metric (device={device})...")
    ...
    print(f"  Segmenter backend: {seg_backend}")
    print(f"  Detector backend:  {det_backend}")
    ...
    result = obj.compute()
    dt = time.time() - t0
    return result, f"seg={seg_backend},det={det_backend}", dt

def main():
    p = argparse.ArgumentParser(description="Smoke-test M3 Background metric")
    ...
    print("\n" + "=" * 90)
    print("  M3 Background Metric — Standalone Smoke-Test")
    ...
    try:
        result, backend, dt = _probe_m3(args.device, batches)
        print(f"\n  {_g('LOADED')}  M3 Background  ({backend}, {dt:.1f}s)")
        print("  Computed values:")
        for k in sorted(result):
            print(f"    {k:40s} = {_fv(result[k])}")
        status = 0
    except Exception as e:
        print(f"\n  {_r('NOT LOADED')}  M3 Background")
