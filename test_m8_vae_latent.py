"""
test_m8_vae_latent.py
=====================
Standalone smoke-test for M8 VAE Latent Space Diversity metric.

Pretrained models: stabilityai/sd-vae-ft-mse → CompVis/sd-v1-4 → runwayml/sd-v1-5

Usage
-----
python test_m8_vae_latent.py --max_batches 2
python test_m8_vae_latent.py --device cpu --max_batches 4
"""
...
def _probe_m8(device, batches):
    print(f"\n  Loading M8 VAE Latent metric (device={device})...")
...
def main():
    p = argparse.ArgumentParser(description="Smoke-test M8 VAE Latent metric")
    ...
    print("\n" + "=" * 90)
    print("  M8 VAE Latent Metric — Standalone Smoke-Test")
    ...
    try:
        result, backend, dt = _probe_m8(args.device, batches)
        print(f"\n  {_g('LOADED')}  M8 VAE Latent  (backend={backend}, {dt:.1f}s)")
        print("  Computed values:")
        for k in sorted(result):
            print(f"    {k:40s} = {_fv(result[k])}")
        status = 0
    except Exception as e:
        print(f"\n  {_r('NOT LOADED')}  M8 VAE Latent")
