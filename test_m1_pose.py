...
test_m1_pose.py
===============
Standalone smoke-test for M1 Pose Diversity & Articulation metric.

Pretrained model: KeypointRCNN (torchvision, COCO-17 keypoints)

Usage
-----
python test_m1_pose.py --max_batches 2
python test_m1_pose.py --device cpu --max_batches 4
"""
...
# ── metric probe ──────────────────────────────────────────────────────────────
def _probe_m1(device, batches):
    print(f"\n  Loading M1 Pose metric (device={device})...")
...
# ── main ──────────────────────────────────────────────────────────────────────
def main():
    p = argparse.ArgumentParser(description="Smoke-test M1 Pose metric")
    ...
    print("\n" + "=" * 90)
    print("  M1 Pose Metric — Standalone Smoke-Test")
    ...
    # ── run metric ────────────────────────────────────────────────────────
    try:
        result, backend, dt = _probe_m1(args.device, batches)
        print(f"\n  {_g('LOADED')}  M1 Pose  (backend={backend}, {dt:.1f}s)")
        print("  Computed values:")
        for k in sorted(result):
            print(f"    {k:40s} = {_fv(result[k])}")
        status = 0
    except Exception as e:
        print(f"\n  {_r('NOT LOADED')}  M1 Pose")
