"""
test_m5_body_shape.py
=====================
Standalone smoke-test for M5 Body Shape Diversity metric.

Pretrained models: HMR2.0 (4D-Humans) → ViT-B/16 proxy (timm)

Usage
-----
python test_m5_body_shape.py --max_batches 2
python test_m5_body_shape.py --device cpu --max_batches 4
"""
...
def _probe_m5(device, batches):
    print(f"\n  Loading M5 Body Shape metric (device={device})...")
...
def main():
    p = argparse.ArgumentParser(description="Smoke-test M5 Body Shape metric")
    ...
    print("\n" + "=" * 90)
    print("  M5 Body Shape Metric — Standalone Smoke-Test")
    ...
    try:
        result, backend, dt = _probe_m5(args.device, batches)
        print(f"\n  {_g('LOADED')}  M5 Body Shape  (backend={backend}, {dt:.1f}s)")
        print("  Computed values:")
        for k in sorted(result):
            print(f"    {k:40s} = {_fv(result[k])}")
        status = 0
    except Exception as e:
        print(f"\n  {_r('NOT LOADED')}  M5 Body Shape")
